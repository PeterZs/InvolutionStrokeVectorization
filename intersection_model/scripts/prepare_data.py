...
def copy_and_unpack_tar_data(data_path_str, scratch_dir_str, ignore_existing=False):
    ...
    tar_cmd = [command, "-xf", str(tar_file_in_scratch), "-C", str(dest)]
    print(" ".join(tar_cmd))
    try:
...
def main():
    ...
    for dataset_name in args.datasets:
        ...
        # Paths
        source_tar = os.path.join(DATA_PERMANENT, args.mode, f"{dataset_name}.tar")
        scratch_mode_dir = os.path.join(DEST_PATH, args.mode)
    ...
    print("\nAll processing complete. Data at:\n", dest_dir)
