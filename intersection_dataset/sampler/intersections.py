...
def get_red_black_map(pl, indexes: np.ndarray, intersections: list[IntersectionPoint]) -> dict[int, int]:
    ...
    rb[inter_indexes] = 2
    return {i: val for i, val in enumerate(rb)}
...
def get_trapping_black_indices(nodes: dict[int, int]) -> list[int]:
    '''
    Identifies Black nodes (2) that are bounding a contiguous run of Red nodes (1).
    ...
    # "Start" of sequence is an implicit Black node (None index)
    left_boundary_is_black = True
    left_boundary_index = None
    ...
    for idx in sorted_indices:
        ...
        if val == 1:
            ...
            # Check if this Black node is touching the previous Black node
            # (Consecutive Black nodes)
            is_consecutive = (left_boundary_index is not None) and (left_boundary_index == idx - 1)
            
            # Trigger if:
            # 1. We have a valid left wall AND we found red nodes (Standard Trap)
            # 2. We have a valid left wall AND it is effectively touching this wall (Consecutive)
            ...
            # Neutral node breaks the trap chain
            left_boundary_is_black = False
            left_boundary_index = idx
            has_red_nodes = False

    # Check End of List condition
    # The "End" is an implicit Black node.
    # We only care about this if there are actual Red nodes trapped against the end.
    # (We don't count "2 -> End" as consecutive black nodes).
    if left_boundary_is_black and has_red_nodes:
        if left_boundary_index is not None:
            bounding_indices.add(left_boundary_index)
