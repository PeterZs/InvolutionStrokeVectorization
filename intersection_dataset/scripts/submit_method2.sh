...
for ((offset=START; offset<MAX; offset+=STEP)); do
  ...
  $SBATCH_CMD --wrap="uv run python -u main_method2.py --out_dir ${OUT_DIR_CURR} --from_seed ${offset} --to_seed ${to} --n_add 0 1"
done